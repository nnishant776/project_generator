'''
Project scaffold generator for C/C++ projects
'''
...
class CScaffoldGenerator(ScaffoldGenerator):
    '''
    ...
    def build_scaffold(self):
        ...
        # create a temporary directory and copy the templates to it
        temp_eval_path = mkdtemp(suffix="_projgen")
        copytree(f"{os.path.dirname(__file__)}/../_templates/c/",
        ...
        environment = j2.Environment(loader=j2.FileSystemLoader(
            f"{temp_eval_path}"))
        renderer = environment.get_template('c_meson.build')
        ...
        os.remove(f"{temp_eval_path}/c_meson.build")
